...
{
  char temp_rvs[MAX_LABEL_LENGTH] = {0};
  int curr_two_bits; /* Holds the current 2-bit chunk extracted from 'number' */
  int j;             /* Index for writing characters into 'dest' */
  j = 0;
  ...
  while (number > 0)
  {
    curr_two_bits = number & MASK_TWO_BITS;                /* Keep only the last two bits */
    temp_rvs[j] = (char)(curr_two_bits + UP_TO_CHARACTER); /* Map 0–3 to 'a'–'d' */
    number = number / 4;
    j++;
  }
